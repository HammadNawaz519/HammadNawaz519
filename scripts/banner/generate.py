...
ROWS = [
    ("Subject", "Hammad Nawaz"),
    ("Role", "Full-Stack Engineer · Systems Developer"),
    ("Origin", "Pakistan"),
    ("Education", "Computer Science · Self-Taught & Shipping"),
    ("Status", "Building + Architecting + Shipping"),
    ("ToolChain", "VS Code · Git · Docker · QEMU"),
    ("Core.Lang", "TypeScript · JavaScript · C · Python · SQL"),
    ("Core.Frontend", "Next.js · React · Tailwind · PWA · Capacitor"),
    ("Core.Backend", "Node.js · Socket.IO · WebRTC · REST APIs"),
    ("Core.Database", "PostgreSQL · Supabase · Upstash Redis"),
    ("Core.Infra", "Vercel · Railway · Render · Cloud"),
    ("Grid.Mail", "hammadnawaz519@gmail.com"),
    ("Grid.LinkedIn", "/in/hammadnawaz519"),
    ("Grid.GitHub", "HammadNawaz519"),
    ("Grid.Live", "myconnectapp.vercel.app"),
]
...
def make_logos() -> dict[str, Image.Image]:
    ...
    # React atom logo: center circle + 3 rotated orbital rings
    react = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    d = ImageDraw.Draw(react)
    d.ellipse((175, 175, 225, 225), fill="black")
    for rot_deg in (0, 60, 120):
        ring = Image.new("RGBA", (size, size), (0, 0, 0, 0))
        rd = ImageDraw.Draw(ring)
        rd.ellipse((60, 155, 340, 245), outline="black", width=20)
        ring_rot = ring.rotate(rot_deg, resample=Image.Resampling.BICUBIC, center=(200, 200))
        react = Image.alpha_composite(react, ring_rot)
    logos["react"] = react
...
def portrait_points(theme: str, rng: np.random.Generator) -> np.ndarray:
    ...
    if w == 1024 and h == 1024:
        crop = source.crop((185, 105, 846, 855)).resize((300, 340), Image.Resampling.LANCZOS)
    elif w == 408 and h == 612:
        crop = source.crop((18, 28, 390, 450)).resize((300, 340), Image.Resampling.LANCZOS)
    else:
        crop_h = int(h * 0.75)
        crop_w = int(crop_h * 300 / 340)
        crop_x = max(0, (w - crop_w) // 2)
        crop_y = int(h * 0.1)
        crop = source.crop((crop_x, crop_y, crop_x + crop_w, crop_y + crop_h)).resize((300, 340), Image.Resampling.LANCZOS)
...
def render_svg(
    ...
    parts: list[str] = [
        ...
        '<title id="title">Hammad Nawaz — Live System Profile</title>',
    ...
    parts.extend(
        [
            ...
            f'<rect x="962" y="94" width="166" height="24" rx="12" fill="{t["chrome"]}" opacity=".16" '
            f'stroke="{t["chrome"]}"/>',
            f'<text x="1045" y="111" text-anchor="middle" fill="{t["chrome"]}" '
            'font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="13" '
            'font-weight="700">@HammadNawaz519</text>',
        ]
    )
