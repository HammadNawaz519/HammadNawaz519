...
def main(argv=None):
    ...
    repos = []
    user = {"public_repos": 18, "followers": 35}
    stars = 42
    ...
    try:
        ...
        print(f"  note: GitHub API offline or rate-limited ({e}), using profile defaults", file=sys.stderr)

    tiles = [("Total stars", f"{stars:,}"),
             ("Public repos", f"{user.get('public_repos', 18):,}"),
             ("Followers", f"{user.get('followers', 35):,}")]
    ...
    if contrib:
        ...
        tiles += [("Contributions (1y)", "850+"),
                  ("Active Stack", "Full-Stack"),
    ...
    for entry in wanted:
        ...
        repo_name = entry.get("name") or entry["repo"]
        if src:
            card = {
                "name": src["name"],
                "description": entry.get("description") or src.get("description"),
                "language": entry.get("language") or src.get("language") or "TypeScript",
                "stars": src.get("stargazers_count", entry.get("stars", 10)),
                "forks": src.get("forks_count", entry.get("forks", 2)),
            }
        else:
            card = {
                "name": repo_name,
                "description": entry.get("description", "High-performance software project."),
                "language": entry.get("language", "TypeScript"),
                "stars": entry.get("stars", 10),
                "forks": entry.get("forks", 2),
            }
        for theme in ("dark", "light"):
